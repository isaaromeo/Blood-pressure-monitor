...
void print_magnitude_vector(const char *title, complex *x, int n) {
  ...
  printf("%s (dim=%d):", title, n);
  for(i=0; i<n; i++ ) {
    real magnitude = sqrt(powf(x[i].Re, 2) + powf(x[i].Im, 2));
    printf(" %5.2f ", magnitude);
  }
  putchar('\n');
  return;
}
...
float get_highest_harmonic(complex *v, int n, float fs){
  ...
  for (size_t i = 12; i < 25; i++){ // Start at i=1 to get rid of DC

    real magnitude = sqrt(powf(v[i].Re, 2) + powf(v[i].Im, 2));
    if (magnitude > A_max){
      A_max = magnitude;
      arg_max = i;
    }
  ...
  return (float)arg_max*fs/(float)n;
}
